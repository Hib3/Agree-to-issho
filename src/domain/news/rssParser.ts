...
export function parseRss2Json(
  ...
  const feed =
    payload.feed && typeof payload.feed === "object" ? (payload.feed as Record<string, unknown>) : {};
...
function parseEntry(
  ...
  const linkElement = elements(entry, "link").find(
...
function resolveXmlBase(element: Element, fallback: string) {
  ...
  return chain.reduce((base, part) => {
    const value =
      part.getAttributeNS("http://www.w3.org/XML/1998/namespace", "base") ?? part.getAttribute("xml:base");
...
function parseFeedDate(
  value: string,
  fetchedAt: number
): { timestamp: number; status: NonNullable<NewsItem["dateStatus"]> } {
...
function feedFingerprint(items: NewsItem[]) {
  return hashText(
    items
      .slice(0, 5)
      .map((item) => `${normalizeNewsUrl(item.url)}\u0000${item.title}`)
      .join("\u0001")
  );
}
