...
export function generateDiary(input: {
  ...
  const todaysMemories = input.memories.filter(
    (memory) => new Date(memory.createdAt).toISOString().slice(0, 10) === input.date
  );
  const todaysDialogue = input.dialogue.filter(
    (turn) => new Date(turn.createdAt).toISOString().slice(0, 10) === input.date
  );
  const conceptIds = Array.from(
    new Set([
      ...todaysMemories.flatMap((memory) => memory.conceptIds),
      ...todaysDialogue.flatMap((turn) => turn.conceptIds)
    ])
  ).slice(0, 4);
  ...
  const learnedIds = new Set(
    todaysMemories.filter((memory) => memory.type === "word_learned").flatMap((memory) => memory.conceptIds)
  );
  const learnedNames = conceptIds
    ...
    .filter((concept): concept is Concept => Boolean(concept))
    .map(displayConcept);
  const corrected = todaysMemories.some(
    (memory) => memory.type === "player_choice" && memory.payload.effect === "deny"
  );
  ...
  if (learnedNames.length)
    parts.push(`新しく覚えたのは${learnedNames.map((name) => `「${name}」`).join("と")}っ！`);
