...
export function memorySalience(
  ...
  const density =
    relations.filter(
      (relation) =>
        memory.conceptIds.includes(relation.fromConceptId) || memory.conceptIds.includes(relation.toConceptId)
    ).length * 2;
  ...
  const recentRecallPenalty =
    memory.lastRecalledAt && context.now - memory.lastRecalledAt < 3_600_000 ? 30 : 0;
  const ageDays = Math.max(0, context.now - memory.createdAt) / 86_400_000;
  const ageDecay = Math.min(18, ageDays * 0.45);
  return (
    memory.importance * 20 + preference + density + location + unanswered - recentRecallPenalty - ageDecay
  );
}
