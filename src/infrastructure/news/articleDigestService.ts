...
export function buildFeedDigest(item: NewsItem, now = Date.now()): ArticleDigest {
  ...
  if (item.feedContent) evidence.push(...sentences(item.feedContent, 3).map((text, index) => ({
    id: `${item.id}_feed_${index}`,
    text,
    source: "feed_content" as const
  })));
  return digestFromEvidence(item, evidence, contentLevel, item.url, now, contentLevel === "headline_only"
    ? ["見出し以外の内容は取得できていません。"]
...
export async function fetchArticleDigest(
  ...
  let directFailure = "";
  try {
    const response = await fetchArticleText(item.url, "text/html, application/xhtml+xml, text/plain", options.signal);
...
function digestFromEvidence(
  ...
  const numericalFacts = evidence.flatMap((entry) =>
    Array.from(entry.text.matchAll(/(?:約|およそ|最大|最低)?\s*[0-9０-９][0-9０-９,.，]*\s*(?:人|件|回|年|月|日|時|分|秒|％|%|円|ドル|キロ|km|駅|社|か所|カ所)?/gu))
      .slice(0, 3)
  ...
  return {
    ...
    entities,
    topics,
    events: keyFacts.slice(0, 2).map((fact, index) => ({ id: `${item.id}_event_${index}`, description: fact.text, evidenceId: fact.evidenceId })),
    numericalFacts,
    uncertainties,
    tone,
    confidence: contentLevel === "article_extract" ? 0.78 : contentLevel === "feed_content" ? 0.65 : contentLevel === "feed_summary" ? 0.5 : 0.25
  };
}
...
function extractArticleText(source: string, contentType: string) {
  ...
  document.querySelectorAll("nav, aside, footer, form, dialog, script, style, noscript, iframe, .advertisement, .ads, .social, .share, .comments, .related, .cookie, [role='navigation'], [aria-label*='広告']").forEach((node) => node.remove());
...
function extractReaderText(markdown: string) {
  const lines = markdown.split(/\r?\n/u)
    .map((line) => line.replace(/^#{1,6}\s+/u, "").replace(/!\[[^\]]*\]\([^)]+\)/gu, "").replace(/\[([^\]]+)\]\([^)]+\)/gu, "$1").trim())
    .filter((line) => line.length >= 20 && !/^(?:Title|URL Source|Published Time|Markdown Content):/iu.test(line));
...
async function fetchArticleText(url: string, accept: string, parentSignal?: AbortSignal) {
  ...
  if (parsed.protocol !== "https:" || parsed.username || parsed.password) throw new Error("公開されたHTTPSの記事だけ取得できます。");
  ...
  try {
    ...
    return { text, finalUrl: response.url || parsed.href, contentType: response.headers.get("content-type") ?? "" };
  } finally {
...
function classifyTone(text: string): ArticleTone {
  if (/(死亡|死者|亡くな|重大事故|災害|地震|戦争|犯罪|逮捕|病気|医療|自傷|自殺|差別|選挙|政府|国会|首相|大統領)/u.test(text)) return "sensitive";
...
function classifyTopics(text: string) {
  ...
  const matched = definitions.filter(([pattern]) => pattern.test(text)).map(([, key, label]) => ({ key, label }));
  return matched.length > 0 ? matched.slice(0, 3) : [{ key: "general", label: "世の中の出来事" }];
}

function extractEntities(text: string) {
  const matches = Array.from(text.matchAll(/[一-龠々ァ-ヶーA-Za-z0-9]{2,20}(?:市|県|町|村|駅|大学|会社|庁|省|党|チーム|研究所)/gu));
  const unique = [...new Set(matches.map((match) => match[0]))].slice(0, 8);
  return unique.map((name) => ({
    name,
    kind: /(?:市|県|町|村|駅)$/u.test(name) ? "place" as const : /(?:会社|庁|省|党|チーム|大学|研究所)$/u.test(name) ? "organization" as const : "other" as const
  }));
}

function userArticleFailure(error: unknown) {
  if (error instanceof DOMException && error.name === "AbortError") return "記事の取得が時間切れになりました。";
