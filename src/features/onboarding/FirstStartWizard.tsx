...
export function FirstStartWizard({ onComplete }: { onComplete: () => Promise<void> }) {
  ...
  async function finish() {
    ...
    try {
      ...
      setError("この端末へ名前を保存できませんでした。空き容量を確認して、もう一度試してください。");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <main className="wizard-screen">
      <CharacterStage
        emotion={step === "hello" ? "happy" : "curious"}
        locationId="room"
        timeOfDay="day"
        isSpeaking
      />
      ...
      <section className="wizard-panel">
        ...
        {step === "hello" ? (
          <button className="primary" type="button" onClick={() => setStep("name")}>
            はじめまして
          </button>
        ) : null}
        {step === "name" ? (
          <form
            onSubmit={(event) => {
              ...
              if (safePlayerText(name, 16)) {
                setCallName(safePlayerText(name, 16));
                setStep("call");
              }
            }}
          >
            ...
            <button className="primary" type="submit" disabled={!safePlayerText(name, 16)}>
              次へ
            </button>
          </form>
        ) : null}
        {step === "call" ? (
          ...
          >
            ...
            <button className="primary" type="submit" disabled={busy || !safePlayerText(callName, 16)}>
              最初の言葉を教える
            </button>
          </form>
        ) : null}
