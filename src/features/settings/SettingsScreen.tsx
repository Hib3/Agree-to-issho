...
export function SettingsScreen({
  ...
  async function patch(changes: Partial<GameSettings>) {
    const optimistic = { ...settings, ...changes, updatedAt: Date.now() };
    ...
    try {
      ...
      setNewsStatus(
        result.candidates.length === 1
          ? "RSS候補を1件見つけました。内容を確認して追加してください。"
          : `RSS候補を${result.candidates.length}件見つけました。追加するものを選んでください。`
      );
    } catch (error) {
  ...
  async function addCandidate(candidate: NewsFeedCandidate) {
    ...
    try {
      ...
      const next = {
        ...latest,
        newsEnabled: true,
        newsFeeds: [...latest.newsFeeds, feed],
        updatedAt: Date.now()
      };
      await db.settings.put(next);
      setFeedCandidates([]);
  ...
  async function setFeedEnabled(feedId: string, enabled: boolean) {
    const latest = (await db.settings.get("local")) ?? settings;
    await patch({
      newsFeeds: latest.newsFeeds.map((feed) => (feed.id === feedId ? { ...feed, enabled } : feed))
    });
  }
      ...
      <section className="settings-list paper-panel">
        ...
        <label>
          ...
          <select
            ...
            onChange={(event) => void patch({ textSpeed: event.target.value as GameSettings["textSpeed"] })}
          >
        ...
        <label>
          ...
          <select
            ...
            onChange={(event) => void patch({ fontScale: event.target.value as GameSettings["fontScale"] })}
          >
        ...
        <label className="toggle-row">
          <input
            ...
            onChange={(event) => void patch({ highContrast: event.target.checked })}
          />
          高いコントラスト
        </label>
        <label className="toggle-row">
          <input
            ...
            onChange={(event) => void patch({ reducedMotion: event.target.checked })}
          />
          動きを減らす
        </label>
        <label className="toggle-row">
          <input
            type="checkbox"
            checked={settings.autonomousSpeech}
            onChange={(event) => void patch({ autonomousSpeech: event.target.checked })}
          />
          アグリちゃんから話しかける
        </label>
        ...
        <section className="settings-section" aria-labelledby="sound-settings-title">
          ...
          <button
            className="quiet settings-preview"
            type="button"
            disabled={settings.muted || settings.volume <= 0}
            onClick={() => playGameSound("confirm", settings)}
          >
            <Volume2 aria-hidden="true" />
            音を試す
          </button>
          <p className="settings-help">
            会話開始、ページ送り、決定時の短い効果音です。声の読み上げではありません。
          </p>
        </section>
        ...
        <section className="settings-section news-settings" aria-labelledby="news-settings-title">
          ...
          <label className="toggle-row">
            <input
              ...
              onChange={(event) => void patch({ newsEnabled: event.target.checked })}
            />
            ニュース機能を使う
          </label>
          <label>
            自動更新
            <select
              value={settings.newsRefreshMinutes}
              onChange={(event) =>
                void patch({
                  newsRefreshMinutes: Number(event.target.value) as GameSettings["newsRefreshMinutes"]
                })
              }
            >
          ...
          >
            ...
            <button className="quiet" type="submit" disabled={!feedUrl.trim() || refreshing}>
              ...
              {feedCandidates.map((candidate) => (
                <li key={candidate.canonicalUrl}>
                  <span>
                    ...
                    <small>
                      {candidate.format ? candidate.format.toUpperCase() : "形式未確認"}・
                      {candidate.validation === "valid"
                        ? "内容確認済み"
                        : candidate.validation === "unverified"
                          ? "ブラウザでは未確認"
                          : "無効"}
                    </small>
                    {candidate.latestArticle ? <small>最新例: {candidate.latestArticle}</small> : null}
                  </span>
                  <button
                    className="quiet"
                    type="button"
                    disabled={refreshing || candidate.validation !== "valid"}
          ...
          <label className="toggle-row">
            <input
              type="checkbox"
              checked={settings.newsUseFeedDiscoveryHelper}
              onChange={(event) => void patch({ newsUseFeedDiscoveryHelper: event.target.checked })}
            />
            サイトからRSSを探す補助を使う
          </label>
          <p className="settings-help">
            許可すると、入力したサイトURLを feedsearch.dev へ送る場合があります。
          </p>
          <label className="toggle-row">
            <input
              type="checkbox"
              checked={settings.newsUseFeedFetchHelper}
              onChange={(event) => void patch({ newsUseFeedFetchHelper: event.target.checked })}
            />
            直接読めないRSSの取得補助を使う
          </label>
          <p className="settings-help">
            許可すると、登録したRSS URLを rss2json.com または jina.ai
            へ送る場合があります。保存するのは見出しと短い説明だけです。
          </p>
          <label className="toggle-row">
            <input
              type="checkbox"
              checked={settings.newsUseArticleHelper}
              onChange={(event) => void patch({ newsUseArticleHelper: event.target.checked })}
            />
            記事本文の取得補助を使う
          </label>
          <p className="settings-help">
            記事を選んだ時だけ、直接読めなかった記事URLを jina.ai へ送ることを許可します。
            <br />
            <a href="https://feedsearch.dev" target="_blank" rel="noreferrer">
              RSS検索: Feedsearch
            </a>{" "}
            /{" "}
                  ...
                  <label className="toggle-row">
                    ...
                    <span>
                      ...
                      <small>
                        {feed.lastError ??
                          (feed.lastSuccessAt
                            ? `最終更新 ${new Date(feed.lastSuccessAt).toLocaleString("ja-JP")}`
                            : "まだ更新していません")}
                      </small>
                    </span>
                  </label>
          ...
          <button
            className="quiet"
            type="button"
            disabled={refreshing || settings.newsFeeds.every((feed) => !feed.enabled)}
            onClick={() => void refreshFeeds()}
          >
            <RefreshCw aria-hidden="true" />
            {refreshing ? "確認中…" : "今すぐ更新"}
          </button>
          {newsStatus ? (
